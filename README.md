...
**Рекомендуемый способ установки:** использовать `aftercall.skill` или целую папку `aftercall/`.

**Не рекомендуется** копировать вручную только `SKILL.md`, потому что skill использует ещё `references/`, `scripts/` и другие файлы. Без них часть логики может работать некорректно или не работать вообще.

### Вариант 1: установить в текущий workspace агента

Самый безопасный путь — скачать весь репозиторий или взять готовый `aftercall.skill`, а не копировать один файл вручную.
